...
<img src = "./image/의사결정나무/트리 구조.JPG" width="50%">
...
- **재귀적 분기 : Recursive Partitioning**
    - 입력변수의 영역을 두 개로 구분 $\rightarrow$ 구분하기 전보다 구분된 뒤에 **각 영역의 순도(purity, homogeneity)가 증가**하도록
    - 하나의 영역에 몰려있도록 하는 분기점 찾기
- **가지치기 : Pruning the Tree**
    - **과적합을 방지**하기 위해 너무 자세하게 구분된 영역을 통합
- 재귀적 분기 $\rightarrow$ 가지치기(사후적 가지치기)
...
- 전체 영역을 어떻게 분할해야 할까?
    - 각 노드에 한 범주에 속하는 객체들이 다수 존재하도록 분할하는 것이 좋다. $Rightarrow$ 동질성 최대화
...
<img src = "./image/의사결정나무/한 영역에 대한 지니계수.JPG" width="80%">

- 두 범주만 존재하는 상황에서 영역 A의 모든 객체들이 동일한 범주에 속할 경우 : I(A) = 0
    - $1-(\frac{1}{1})^2 - (\frac{0}{1})^2 = 1-1-0 = 0$
- 두 범주만 존재하는 상황에서 영역 A에 속하는 객체들이 1/2의 비중으로 존재할 경우 : I(A) = 0.5 $\Rightarrow$ 혼잡도가 가장 높은 경우
    - $1-(\frac{1}{2})^2 - (\frac{1}{2})^2 = 1-\frac{1}{4}-\frac{1}{4} = 0.5$
    
#### 두 개 이상의 영역에 대한 지니 계수 (분할 후 지니계수) 
<img src = "./image/의사결정나무/두 개 이상의 영역에 대한 지니계수.JPG" width="80%">

- 분기 후의 **정보획득(Information gain)** : 0.4688 - 0.3438 = **0.1250** $\Rightarrow$ 분기 기준(빨잔 점선)의 효과
...
2. 순차적으로 가능한 분기점에 대한 정보획득을 계산한다.
    ...
    - 분기 전 : $1-(\frac{12}{24})^2 - (\frac{12}{24})^2 = 0.5000$ $\Rightarrow$ 최악의 상황!(반반)
    - 분기 후 : $\frac{1}{24} x (1-(\frac{0}{1})^2 - (\frac{1}{1})^2) + \frac{23}{24} x (1-(\frac{12}{23})^2 - (\frac{11}{23})^2) = 0.4783$
    - 정보 획득 : $0.5000 - 0.4783 = 0.0217$
<img src = "./image/의사결정나무/첫번째 후보 분기점 표.JPG" width="20%">
<center> [Lot size 정렬 표] </center>
<img src = "./image/의사결정나무/첫번째 후보 분기점 그래프.JPG" width="60%">
...
<img src = "./image/의사결정나무/첫번째 분기점 그래프.JPG" width="50%">


5. **모든 노드의 순도가 100%가 될 때까지 분기를 수행한다.** 
    - 정보획득이 0이 되는 시점까지 수행
    - ★한 번의 분기로 끝나는 것이 아니라 여기서부터 다시 시작해서 똑같은 작업을 수행한다. $\Rightarrow$ **재귀적**
    - 두 번째 분기점 : Income 84.75
<img src = "./image/의사결정나무/두번째 분기점 총.JPG" width="100%">


6. 재귀적 분기 완료
    - 모든 말단 노드에는 한 범주의 객체들만 존재한다.
    - Full Tree 완성 $\rightarrow$ 순도 100% $\Rightarrow$ Training Data Error = 0 (완벽 구분)
<img src = "./image/의사결정나무/재귀적 분기 완료 그래프.JPG" width="40%">
